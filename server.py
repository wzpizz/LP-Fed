...
class Server():
    ...
    def train(self, epoch ,cdw, use_cuda):
        ...
        print("num_of_clients",self.num_of_clients)
        ...
        if cdw:
            ...
            # print(weights)
        self.current_client_list1=current_client_list
        self.model1s=models
    ...
    def test(self, use_cuda):
        ...
        for dataset in self.data.datasets:
            ...
            with torch.no_grad():
                ...
                print("aa=",gallery_feature.shape)
    ...
    def localmodeltest(self, use_cuda):
        modelss=self.model1s
        print("=" * 10)
        print("Local model Start Tesing!")
        print("=" * 10)
        print('We use the scale: %s' % self.multiple_scale)
        ...
        for dataset in self.data.datasets:
            ...
            for i in range( len(modelss)):
                if dataset==self.current_client_list1[i]:
                    federated_model=modelss[i]
                    break
            federated_model = federated_model.eval()
            if use_cuda:
                federated_model = federated_model.cuda()

            with torch.no_grad():
                gallery_feature = extract_feature(federated_model, self.data.test_loaders[dataset]['gallery'],
                                                  self.multiple_scale)
                query_feature = extract_feature(federated_model, self.data.test_loaders[dataset]['query'],
                                                self.multiple_scale)
